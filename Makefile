...
uv.lock: ## Locks dependencies and updates uv.lock on the local file system
	...
	docker compose --progress=plain run --rm dev sh -c "cp -f /tmp/uv.lock /usr/src/langchain_ai_skills_framework/uv.lock"
...
update-fast: ## Re-locks uv.lock quickly using an existing container (skips full rebuild)
	...
	docker compose run --no-deps --name skills_framework_uv_lock dev sh -c "uv lock --refresh && cp uv.lock /tmp/uv.lock.out" && \
...
build: ## Builds the package
	...
	docker compose run --rm --name langchain_ai_skills_framework dev python3 -m build
